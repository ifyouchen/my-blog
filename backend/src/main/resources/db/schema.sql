...
CREATE TABLE `blog_article` (
    ...
    `status`          varchar(20)     NOT NULL DEFAULT 'DRAFT' COMMENT '状态：DRAFT-草稿 SCHEDULED-定时发布 PUBLISHED-已发布 OFFLINE-已下架 DELETED-已删除',
    `scheduled_publish_at` datetime   NULL DEFAULT NULL COMMENT '计划发布时间',
    ...
    INDEX `idx_article_scheduled_publish` (`status`, `scheduled_publish_at`, `deleted_at`) USING BTREE,
...
CREATE TABLE `blog_comment` (
    ...
    `content`         varchar(1000)   NOT NULL COMMENT '评论内容（支持 Markdown）',
    `quote_text`      varchar(300)    NULL COMMENT '引用原文',
    `quote_prefix`    varchar(80)     NULL COMMENT '引用前文',
    `quote_suffix`    varchar(80)     NULL COMMENT '引用后文',
...
CREATE TABLE `blog_message` (
    ...
    `parent_id`       bigint unsigned DEFAULT NULL COMMENT '被回复消息ID',
